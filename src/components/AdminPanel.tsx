...
import { Plus, Edit2, Trash2, ArrowLeft, LogOut, Layout, Database } from 'lucide-react';
import { getFirebaseConfig, setDbConfig } from '../firebase';
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              ...
              className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg font-bold transition-colors"
            >
...
const FirebaseConfigModal = ({ onClose }: { onClose: () => void }) => {
  const [configStr, setConfigStr] = useState(() => JSON.stringify(getFirebaseConfig(), null, 2));

  const handleSave = () => {
    try {
      const config = JSON.parse(configStr);
      setDbConfig(config);
      alert("Firebase sozlamalari saqlandi! Sahifani yangilang.");
      onClose();
    } catch(e) {
      alert("Xato JSON formati!");
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#111] border border-purple-500/30 p-6 rounded-2xl w-full max-w-2xl">
        <h2 className="text-2xl font-bold mb-4 text-purple-500">Firebase Sozlamalari</h2>
        <p className="text-gray-400 text-sm mb-4">
          O'zingizning Firebase loyihangiz ma'lumotlarini (JSON formatida) kiriting. 
          Agar kiritmasangiz, barcha o'zgarishlar vaqtinchalik brauzer xotirasida (Local Storage) saqlanadi.
        </p>
        <textarea 
          value={configStr}
          onChange={e => setConfigStr(e.target.value)}
          className="w-full h-64 bg-black border border-white/10 p-4 rounded font-mono text-sm text-green-400 outline-none"
        />
        <div className="flex justify-end gap-4 mt-6">
          <button onClick={onClose} className="px-4 py-2 bg-gray-700 rounded hover:bg-gray-600">Bekor qilish</button>
          <button onClick={handleSave} className="px-4 py-2 bg-purple-600 rounded hover:bg-purple-500 font-bold">Saqlash</button>
        </div>
      </div>
    </div>
  );
